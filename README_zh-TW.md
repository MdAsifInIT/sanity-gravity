...
*   **📸 完美快照 (Perfect Snapshots)**: 將容器的完整狀態 (檔案系統、已安裝應用、登入狀態) 儲存為 Docker Image。隨時從這個凍結的狀態分叉 (Fork) 出新專案。
...
./sanity-cli up -v [name]   # 建立並啟動容器 (docker compose up)
  # 選項:
  ...
  #   --image [tag]       (使用自訂的基礎映像檔進行 Snapshot 還原)
...
./sanity-cli ide <action>   # 容器內 IDE 維護 (update/reinstall)
./sanity-cli upgrade        # [遺留] 將舊容器遷移至新架構
./sanity-cli sync_config    # 同步設定至運行中容器 (Runtime Sync)
./sanity-cli snapshot       # 建立容器快照
```
...
Sanity-Gravity 內建了一個智慧型的 Proxy 管理器，用於在主機與容器之間安全地橋接 SSH Agent Socket。這使得容器內的 `git` 操作能夠使用主機的私鑰進行認證，而無需將私鑰檔案複製進容器。

通常 `sanity-cli up` 會自動處理 Proxy 的啟動與掛載。但在某些情況下 (如 Daemon 意外停止或路徑變更)，您可能需要手動管理它：

```bash
# 檢查 Proxy 狀態 (包含 Socket 路徑與連線測試)
./sanity-cli proxy status

# 手動啟動/修復 Proxy 服務
./sanity-cli proxy setup

# 移除 Proxy 服務 (清理 Socket 與 Systemd Service)
...
停止重複的設定步驟！您可以將當前的容器狀態「凍結」成一個新的映像檔，並將其作為未來實例的基底。

**使用情境 (Scenario)**: 您在 `my-base-env` 容器中手動安裝了複雜的開發環境或登入了網頁服務，現在您想基於這個完全相同的狀態，分岔 (Fork) 出一個新的實驗專案 `my-new-project`。

1.  **建立快照 (Create Snapshot)**:
    這將會暫停容器，將其 Commit 為新的 Docker Image，並驗證完整性。
    ```bash
...
2.  **使用快照 (Fork)**:
    使用該快照作為基底啟動新專案。
    > **注意**: 即使使用自訂 Image，您仍需指定版本參數 (`-v kasm` 或 `-v vnc`) 以確保載入正確的桌面環境設定。
    > **警告**: 從快照啟動時，系統將會**重置 Antigravity Agent 的瀏覽器 Profile** (包含 Cookies/Session) 以確保穩定性。
    ```bash
    # 基於快照啟動新實例 'my-new-project'
    ./sanity-cli up -v kasm --name my-new-project --image my-verified-state:v1
    ```

### 🛠️ IDE 維護與安全升級 (Gravity-CLI)

Sanity-Gravity 內建了針對 `apt upgrade` 導致 IDE 解除安裝或崩潰的防護機制。

我們將宿主機管理與容器內部軟體管理進行了嚴格的切割：
- **Host (宿主機)**：`sanity-cli` 負責管理容器的生命週期。
- **Inside (容器內)**：`gravity-cli` (內建腳本) 負責安全地管理 Antigravity IDE，確保 `--no-sandbox` 啟動防護不會被破壞。

#### 從宿主機 (使用 Sanity-CLI)
如果您遇到 IDE 崩潰 (例如 Google Gemini 強制更新帶來的災情) 或者只是想安全地將 IDE 更新到最新版，請在宿主機使用代理的 `ide` 指令。
> **注意**: `--name` 參數用於指定目標容器實例 (預設為 `sanity-gravity`)。若您同時運行多個隔離環境，請務必指定欲維護的專案名稱。

```bash
# 安全地透過 apt 將 Antigravity IDE 更新至最新版
./sanity-cli ide update --name sanity-gravity

# 核彈級修復：徹底清除並重新乾淨安裝 IDE 以解決持續性的崩潰問題
./sanity-cli ide reinstall --name sanity-gravity
```
*(這些指令會自動在目標容器內部以 root 身分呼叫 `gravity-cli` 腳本，保持您的宿主機環境乾淨無染。)*

#### 在容器內 (使用 Gravity-CLI)
如果您已經身處容器的 shell 中 (例如透過 `./sanity-cli shell`)，您可以直接使用 `gravity-cli` 工具。請注意，您必須以 `root` 身分 (例如使用 `sudo`) 來執行這些指令。
